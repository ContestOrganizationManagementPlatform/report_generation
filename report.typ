...
#let csvs_path = config.data_paths.joined
#let create_dict = (k, v) => {
  let dict = (:)
  dict.insert(k, v)
  dict
}
#let answers = csv(config.data_paths.answers)
#let answers_map = (:)
...
#let csvs = (
  ("algebra", "calculus", "discrete", "general", "geometry", "guts", "team").map(key => {
    (key, csvs_path + "/" + key + ".csv")
  }) + (("overall", config.data_paths.overall), ("power", config.data_paths.power))
).map(((key, path)) => {
...
#for (test, id_data) in csvs.pairs().map(((test, csv_data)) => {
  ...
  (
    test,
    csv_data.slice(1).map(row => {
      // csv_data.slice(1).slice(0, count: 10).map(row => {
      create_dict(
        row.at(id_index),
        keys.zip(row).map(((k, v)) => {
          create_dict(k, v)
        }).sum(),
      )
    }).sum(),
...
#let json_path = config.data_paths.stats
#let stats = ("algebra", "calculus", "discrete", "general", "geometry", "guts", "team", "overall", "power").map(key => {
  create_dict(key, json(json_path + "/" + key + ".json"))
}).sum()
...
// #{
//   let team_registration = csv(config.data_paths.team_registration)
...
#let get_message = id => {
  ...
  [
    ...
    Thank you again for your participation in SMT 2024! We loved seeing you at our competition,
    and we hope to see you again at SMT 2025! As a reminder, if you have any questions --
    either about the score report or the tournament in general --
    please contact us at #link("stanford.math.tournament@gmail.com").
...
#let get_individual_tests = id => {
  ...
  figure(
    table(columns: (1fr, 1fr), inset: 10pt,
      "", [*Score*],
...
#let get_overall_statistics = () => {
  ...
  figure(
    table(columns: (1fr,) * 7, inset: 10pt,
      "", [*1st Quartile*], [*Median*], [*3rd Quartile*], [*Standard Deviation*], [*HM (top 25%) Cutoff*], [*DHM (top 10%) cutoff*],
      ..stats.pairs().map(((test, data)) => {
        if test == "overall" {
          test = "SMT Index"
        }
        (
          capitalize(test),
          data.at("first_quartile"),
          data.at("median"),
          data.at("third_quartile"),
          data.at("stddev"),
          data.at("third_quartile"),
          data.at("top_10_percent"),
        ).map(v => {
          if type(v) == float {
...
#let get_team_breakdown = team_id => {
  ...
  figure(
    table(columns: (1fr, 1fr, 1fr), inset: 10pt,
      ...
      ..id_map.at(team_id).team.grades.split(",").zip(answers_map.at("team")).enumerate().map((
        (i, (correct, answer)),
      ) => {
...
#let get_guts_breakdown = team_id => {
  ...
  let (t1, t2) = split_at(
    id_map.at(team_id).guts.grades.split(",").zip(answers_map.at("guts")).enumerate().map(((i, (score, answer))) => {
...
#let get_individual_breakdown = id => {
  let (title, test_names, t1, t2) = if id_map.at(id).at("general", default: none) != none {
    let (t1, t2) = split_at(
      id_map.at(id).general.grades.split(",").zip(answers_map.at("general")).enumerate().map((
        (i, (correct, answer)),
      ) => {
        (str(i + 1), mi(answer), capitalize(str(correct)))
      }),
